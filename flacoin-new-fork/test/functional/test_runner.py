...
def main():
    ...
    tmpdir = "%s/flacoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and flacoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "flacoind"]) is not None:
            print("%sWARNING!%s There is already a flacoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "FLACOIND" not in os.environ:
        os.environ["FLACOIND"] = build_dir + '/src/flacoind' + exeext
        os.environ["FLACOINCLI"] = build_dir + '/src/flacoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `flacoin-cli help` (`rpc_interface.txt`).
